...
public static class HintDisplay
{
    ...
    private static readonly ArchipelagoSession Session = Archipelago.HollowKnight.Archipelago.Instance.session;
    private static List<Hint> hints = new();

    private static bool visible = true;
    private const int MaxHints = 20;
    private static int hintsShown = APMapMod.GS.gameplayHints;
    ...
    public static void Build()
    {
        ...
        {
#if DEBUG
            APMapMod.Instance.Log("Creating hint display");
#endif
            layout = new(true, "Hint Display");
            //layout.RenderDebugLayoutBounds = true;
            layout.VisibilityCondition = () =>
                !GUI.worldMapOpen
                && !GUI.quickMapOpen
                && visible;
...
#if DEBUG
            APMapMod.Instance.Log("finished hint display");
#endif
        }
    ...
    private static void ClosePause(On.UIManager.orig_UIClosePauseMenu orig, UIManager self)
    {
        orig(self);
        visible = true;
        UpdateDisplay(APMapMod.GS.gameplayHints);
    }
    ...
    private static void OpenPause(On.UIManager.orig_UIGoToPauseMenu orig, UIManager self)
    {
        orig(self);
        visible = true;
        UpdateDisplay(APMapMod.GS.pauseMenuHints);
    }
...
#if DEBUG
        APMapMod.Instance.Log($"updating {hints.Count} hints");
#endif
...
#if DEBUG
        APMapMod.Instance.Log($"done marking hints on map.");
#endif
        SortHints();
    }
    ...
    {
#if DEBUG
        APMapMod.Instance.Log($"sorting {hints.Count} hints");
#endif
...
#if DEBUG
        APMapMod.Instance.Log("hints sorted");
#endif
        UpdateDisplay();
    }
    ...
    {
#if DEBUG
        APMapMod.Instance.Log($"updating display of {hints.Count}");
#endif
        int shown = 0;
        for (var i = 0; i < hints.Count; i++)
        {
            ...
            if (
                hint.FindingPlayer != Session.ConnectionInfo.Slot ||
                hint.Found || 
                !hint.ItemFlags.HasFlag(ItemFlags.Advancement)
            )
                continue;
            ...
            if (APMapMod.LS.tracker.clearedLocations.Contains(locationString))
                continue;
            ...
            Formatters[x].Text.FontSize = APMapMod.GS.hintFontSize;
            shown++;
            if (shown >= MaxHints)
                break;
        }
#if DEBUG
        APMapMod.Instance.Log($"finished total shown {shown}");
    ...
    private static ColorResult GetColor(long locationID)
    {
        ...
        if (!APMapMod.LS.tracker.uncheckedReachableLocations.Contains(locationName) && !APMapMod.LS.tracker.previewedLocations.Contains(locationName)) return ColorResult.Red;
