...
namespace APMapMod.Map
{
    ...
    internal class MapRooms
    {
...
#if DEBUG
            APMapMod.Instance.Log(obj.name);
            APMapMod.Instance.Log("- " + emd.sceneName);
            APMapMod.Instance.Log("- " + emd.origColor.ToString());
            APMapMod.Instance.Log("- " + emd.origCustomColor.ToString());
#endif
        }
        ...
        public static GameObject CreateExtraMapRooms(GameMap gameMap)
        {
            ...
            foreach (string scene in MainData.GetNonMappedScenes().Where(s => s.IsSpecialRoom()))
            {
                ...
                foreach (Transform roomObj in areaObj.transform)
                {
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if (!color.Equals(Vector4.negativeInfinity) && APMapMod.LS.modEnabled)
                            {
                                sr.color = color;
                            }
                            else
