...
namespace APMapMod.Data
{
    public static class PathfinderData
    {
        ...
        {
            conditionalTerms = JsonUtil.Deserialize<Dictionary<string, string>>("APMapMod.Resources.Pathfinder.Data.conditionalTerms.json");
            adjacentScenes = JsonUtil.Deserialize<Dictionary<string, string>>("APMapMod.Resources.Pathfinder.Data.adjacentScenes.json");
            adjacentTerms = JsonUtil.Deserialize<Dictionary<string, string>>("APMapMod.Resources.Pathfinder.Data.adjacentTerms.json");
            scenesByTransition = JsonUtil.Deserialize<Dictionary<string, string>>("APMapMod.Resources.Pathfinder.Data.scenesByTransition.json");
            transitionsByScene = JsonUtil.Deserialize<Dictionary<string, HashSet<string>>>("APMapMod.Resources.Pathfinder.Data.transitionsByScene.json");
            doorObjectsByScene = JsonUtil.Deserialize<Dictionary<string, string>>("APMapMod.Resources.Pathfinder.Compass.doorObjectsByScene.json");
            doorObjectsByTransition = JsonUtil.Deserialize<Dictionary<string, string>>("APMapMod.Resources.Pathfinder.Compass.doorObjectsByTransition.json");
    
            lmb = new(APMapMod.LS.Context.LM);
    
            foreach ((LogicManagerBuilder.JsonType type, string fileName) in files)
            {
                lmb.DeserializeJson(type, Assembly.GetExecutingAssembly().GetManifestResourceStream($"APMapMod.Resources.Pathfinder.Logic.{fileName}.json"));
            }
    
            // Add godhome logic only for rando 4.0.6 or upwards
            if (APMapMod.LS.Context.LM.Terms.TermLookup.ContainsKey("GG_Waterways"))
            {
            ...
            if (startTerms.Length > 0)
            {
                ...
                //adjacentScenes["Warp-Start"] = "Tutorial_01";
            ...
            waypointScenes = lm.Waypoints.Where(w => RCData.IsRoom(w.Name) || w.Name.IsSpecialRoom()).ToDictionary(w => w.Name, w => w);
        }
    
        public static string[] GetStartTerms()
        {
            // if (APMapMod.LS.Context.InitialProgression is ProgressionInitializer pi && APMapMod.LS.Context.LM is LogicManager lm)
            // {
        ...
        public static string GetAdjacentTerm(this string transition)
        {
            ...
            APMapMod.Instance.LogWarn($"No adjacent term for {transition}");
