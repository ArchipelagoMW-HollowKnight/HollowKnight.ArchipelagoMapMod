...
using PD = APMapMod.Data.PathfinderData;

namespace APMapMod.Data
{
    public static class Pathfinder
    {
        private static TrackerData Td => APMapMod.LS.tracker;
    
        internal static ProgressionManager localPm;
    
        public static void Initialize()
        {
            localPm = new(APMapMod.LS.Context.LM, APMapMod.LS.Context);
        ...
        public static void UpdateProgression()
        {
            ...
            {
                if (!RCData.IsTransition(term.Name)
                    && !RCData.IsRoom(term.Name))
                {
        ...
        public static List<string> ShortestRoute(string start, string final, List<List<string>> rejectedRoutes, bool allowBenchWarp, bool reevaluate)
        {
            ...
            void TryAddNode(SearchNode node, string transition)
            {
                ...
                if (transition.IsSpecialTransition() || normalTransitionSpace.Contains(transition))
                {
                    ...
                    if (adjacent == null || !localPm.lm.Terms.TermLookup.ContainsKey(adjacent)) return;
